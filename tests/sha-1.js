...
import { encode as encodeHex } from '../src/alg/encoding/hex'
...
function testSha1(testcase) {
    const message = new Uint8Array([ ...testcase[1].split('').map(it => it.charCodeAt(0)) ])

    test(`Hash ${testcase[1]}`, () => {
        expect(encodeHex(sha1(message))).toBe(testcase[0].toUpperCase())
    })
}
